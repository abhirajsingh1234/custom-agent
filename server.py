...
from available_github_repositories import get_github_repositories
from available_repository_files import get_github_repository_files
...
available_tools = {
    ...
    "get_github_repositories": "Get all repositories from a GitHub user",
    "get_github_repository_files": "Get all files from a GitHub repository"
}
...
async def execute_tool(request: ExecuteRequest):
    ...
    if tool_name == "send_email":
        ...
        # Extract parameters for getting GitHub repositories.
        username = parameters.get("username")
        
        if not username:
            raise HTTPException(status_code=400, detail="Missing username parameter")
        
        # Call our github_repositories.get_github_repositories function
        result = get_github_repositories(username)
        return result


    elif tool_name == "get_github_repository_files":
        # Extract parameters for getting GitHub repository files.
        username = parameters.get("username")
        repo_name = parameters.get("repo_name")
        
        if not username or not repo_name:
            raise HTTPException(status_code=400, detail="Missing parameters detected at server")
        
        # Call our github_repository_files.get_github_repository_files function
        result = get_github_repository_files(username, repo_name)
        return result
