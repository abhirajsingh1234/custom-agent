...
def get_github_repositories(query,chat_history):
    ...
    available_github_repositories = genai.GenerativeModel(
      model_name="gemini-1.5-flash-8b",
      generation_config=generation_config,
      system_instruction="""
                           Analyze the input query to determine if the user is requesting to fetch GitHub repositories for a specific username.
                            Guidelines:

                            First check if a GitHub username is explicitly provided in the query.
                            if github username is not available in query search for it in chat history.
                            Look for phrases like "get repos for [username]", "fetch GitHub repositories of [username]", etc.
                            Username should match GitHub username format (alphanumeric, hyphens).


                            If username is found:

                            Extract it and format it properly (remove any @ symbols or extra spaces)
                            Check if the username follows GitHub username conventions
                            If multiple possible usernames are mentioned, prioritize the one that appears to be the target


                            Error Handling:

                            Consider potential error cases (invalid username format, API limitations)
                            Include appropriate exception handling for network failures


                            If username is not found:

                            Set get_github_repository to false
                            Set the username parameter to null


                            Always use "get_github_repositories" as the tool value

                            Return JSON response in this format:
                            jsonCopy{
                              "get_github_repository": boolean,  // true only if username is valid
                              "tool": "get_github_repositories",   // constant value
                              "parameters": {
                                "username": string         // extracted GitHub username or null if not found
                              }
                            }
                            get_github_repository can be true only when username is valid and not null.
                            """
    )
    chat_session = available_github_repositories.start_chat(
                history=history
            )
    response = chat_session.send_message(prompt)
    model_response=response.text
    ...
    try:
        ...
        return {"error": f"ERROR OCCURED WHILE FETCHING GITHUB REPOSITORIES, TRY AGAIN"}
    # print("GitHub repositories data:", data)
    return data

def get_github_repository_files(query,chat_history):
    ...
    available_repository_files= genai.GenerativeModel(
      model_name="gemini-1.5-flash-8b",
      generation_config=generation_config,
      system_instruction="""
                            Analyze the input query to determine if the user is requesting to fetch files from a specific GitHub repository.
                            Guidelines:

                            First check if both a GitHub username (owner) and repository name are explicitly provided in the query

                            Look for phrases like "get files from [username]/[repo]", "show files in [username]'s [repo] repository", etc.
                            If GitHub username or repository name is not available in query, search for it in chat history.
                            Username should match GitHub username format (alphanumeric, hyphens)


                            If username and repository are found:

                            Extract them and format properly (remove any @ symbols or extra spaces)
                            Check if they follow GitHub conventions
                            If multiple possible usernames/repositories are mentioned, prioritize those that appear to be the target


                            Optional path parameter:

                            Check if the user specifies a specific directory path within the repository
                            If provided, extract the path and format it properly
                            If not provided, default to the root directory


                            Error Handling:

                            Consider potential error cases (invalid username format, non-existent repository, API limitations)
                            Include appropriate exception handling for network failures


                            If username or repository name is not found:

                            Set get_github_repository_files to false
                            Set the missing parameter(s) to null


                            Always use "get_github_repository_files" as the tool value

                            Return JSON response in this format:
                            jsonCopy{
                              "get_github_repository_files": boolean,  // true only if both username and repo_name are valid
                              "tool": "get_github_repository_files",   // constant value
                              "parameters": {
                                "username": string,       // extracted GitHub username or null if not found
                                "repo_name": string,        // extracted repository name or null if not found
                                "path": string         // extracted path or empty string for root directory
                              }
                            }
                            get_github_repository_files can be true only when owner and repo are valid and not null.
                            """
    )
    chat_session = available_repository_files.start_chat(
                history=history
            )
    response = chat_session.send_message(prompt)
    model_response=response.text
    ...
    try:
        ...
        return {"error": f"ERROR OCCURED WHILE FETCHING GITHUB REPOSITORY FILES, TRY AGAIN"}
    # print("GitHub repository files data:", data)
    return data
...
def get_llm_decision(query, tools, chat_history):
    ...
    elif data['tool'] is not None and data['tool'] == 'get_github_repositories':
        return get_github_repositories(query, chat_history)
    elif data['tool'] is not None and data['tool'] == 'get_github_repository_files':
        return get_github_repository_files(query, chat_history)
...
def main():
    ...
    while True:
        ...
        elif "get_github_repository" in llm_output and llm_output["get_github_repository"] == True:
          if llm_output["parameters"]["username"] is None:
            result = 'context: please provide github username'
          else:
            # Step 5: Instruct the local server to execute the tool.
            result = execute_tool(llm_output["tool"], llm_output["parameters"])
            result = f"\ncontext: {result}"
        ...
        elif "get_github_repository" in llm_output and llm_output["get_github_repository"] == False:
          result = 'context: please provide github username'

        elif "get_github_repository_files" in llm_output and llm_output["get_github_repository_files"] == True:
          if llm_output["parameters"]["username"] is None and llm_output["parameters"]["repo_name"] is None:
            result = 'context: please provide github username and repository'
          elif llm_output["parameters"]["username"] is None:
            result = 'context: please provide github username'
          elif llm_output["parameters"]["repo_name"] is None:
            result = 'context: please provide github repository'
          else:
            # Step 5: Instruct the local server to execute the tool.
            result = execute_tool(llm_output["tool"], llm_output["parameters"])
            result = f"\ncontext: {result}"
        ...
        elif "get_github_repository_files" in llm_output and llm_output["get_github_repository_files"] == False:
          if llm_output["parameters"]["username"] is None and llm_output["parameters"]["repo_name"] is None:
            result = 'context: please provide github username and repository'
          elif llm_output["parameters"]["username"] is None:
            result = 'context: please provide github username'
